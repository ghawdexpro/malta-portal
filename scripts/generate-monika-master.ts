 ...
 *
 * This creates THE master reference that all future Monika images
 * and videos will be based on. Must be approved before proceeding.
 *
 * Run: npx ts-node scripts/generate-monika-master.ts
 */
...
const LOCATION = 'global'; // Pro requires global

const OUTPUT_DIR = path.join(__dirname, '..', 'public', 'images', 'monika');

async function getAccessToken(): Promise<string> {
    // Try JSON credentials from env
...
async function generateImage(prompt: string, aspectRatio: string = '9:16') {
    ...
    console.log(`[NanoBanana Pro] Generating Monika master reference...`);
...
async function main() {
    ...
    // Generate 3 variations for approval
    const prompts = [
        {
...
TECHNICAL: Shot on Sony A7RV, 85mm GM f/1.4, ISO 100. Magazine cover quality. 8K resolution. Zero AI artifacts — must be indistinguishable from real photography.`
    ...
    for (const { name, prompt } of prompts) {
        ...
        try {
            const images = await generateImage(prompt, '3:4'); // Portrait ratio
    ...
    console.log('  That becomes THE reference for all future Monika content.');
    console.log(`${'='.repeat(60)}`);
}
