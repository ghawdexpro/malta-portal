...
function fileUrl(publicPath: string) {
  return `/api/tiktok/files?path=${encodeURIComponent(publicPath)}`;
}
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && entry.name.startsWith('session_')) {
          ...
          const images = files.filter((f) => f.endsWith('_image.png'))
            .map((f) => fileUrl(`videos/asmr/${entry.name}/${f}`));
        ...
        if (!entry.isDirectory() && entry.name.startsWith('monika_asmr_') && entry.name.endsWith('.mp4')) {
          ...
          finalVideos.push({
            url: fileUrl(`videos/asmr/${entry.name}`),
            name: entry.name,
            size: stat.size,
            type: 'video',
            created_at: stat.birthtime.toISOString(),
    ...
    if (fs.existsSync(BEST_DIR)) {
      ...
      for (const file of files) {
        if (file.endsWith('.png') || file.endsWith('.jpg')) {
          ...
          const stat = fs.statSync(filePath);
          bestImages.push({
            url: fileUrl(`images/monika/asmr-best/${file}`),
            name: file,
            size: stat.size,
            type: 'image',
            created_at: stat.birthtime.toISOString(),
